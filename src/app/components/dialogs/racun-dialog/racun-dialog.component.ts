...
export class RacunDialogComponent implements OnInit {
  ...
  public addRacun(): void {
    ...
    (error: Error) => {
      ...
      this.snackBar.open('Doslo je do greske prilikom unosa novog racuna ', 'Zatvori', {
        duration:2500
      })
    }
  }
  public updateRacun(): void {
    ...
    (error: Error) => {
      console.log(error.name + ' ' + error.message);
      this.snackBar.open('Doslo je do greske prilikom modifikacije novog racuna ', 'Zatvori', {
        duration:2500
      })
    }
  }
